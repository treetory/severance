 ...
 * input text 를 담고 있는 영역을 만든다.
 */
const createAttrNameBox = (function(){
    ...
    let main = function(e) {
        ...
        center_div.style.display = "table-cell";
        center_div.style.padding = "10px 10px 10px 10px";
        ...
        if (e.hasOwnProperty("button")) {
            if (e.button.action != undefined || e.button.action != null) {
                let button = document.createElement("button");
                button.innerHTML = e.button.name;
                switch (e.button.action) {
                    default:
                    button.addEventListener("click", function(e){
                        alert("["+e.target.innerHTML+"] 버튼 클릭 이벤트 입니다.");
                    });
                    break;
                }
        ...
        cell_div.appendChild(left_div);
        cell_div.appendChild(center_div);
        cell_div.appendChild(right_div);
